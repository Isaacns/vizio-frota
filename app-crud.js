/* ============================================================
   VIZIO Frota · by VIZIO — CRUD + Cobrança/Bloqueio/Multas + Portal
...
(function(){
  ...
  function enviarLink(locId){const l=byId(W().locatarios,locId);const url=VP.portalUrl(l);
    const msg=`Olá ${l.nome.split(" ")[0]}! Este é o seu acesso exclusivo ao Portal VIZIO Frota — acompanhe seu contrato, vencimentos e multas por aqui:\n${url}\n\nAcesso: use seu login (demo: usuário/senha 1234). VIZIO Frota · A inteligência por trás da frota.`;
  ...
  function avisar(id){const c=W().cobrancas.find(x=>x.id===id);if(!c)return;c.bloqueio="Agendado";const l=byId(W().locatarios,c.locatarioId);
    const msg=`⚠️ VIZIO Frota — Aviso de bloqueio. Olá ${(l.nome||"").split(" ")[0]}, o aluguel do ${vNome(c.veiculoId)} (${c.competencia}) vence ${fmtDate(c.vencimento)}: ${fmtBRL(c.valor)}. Sem a confirmação do pagamento, o veículo será BLOQUEADO via satélite em 24h. 🚗`;
  ...
  function bloquear(id){const c=W().cobrancas.find(x=>x.id===id);if(!c)return;
    ...
    const msg=bloqueado
      ? `✅ VIZIO Frota — Pagamento confirmado. Seu veículo ${vNome(c.veiculoId)} está liberado. Bom trabalho! 🚗`
      : `🔒 VIZIO Frota — O aluguel do ${vNome(c.veiculoId)} (${c.competencia}, ${fmtBRL(c.valor)}) está em atraso. Conforme contrato, o veículo será bloqueado. Regularize via PIX para liberação imediata.`;
    modal(`<h3>${bloqueado?'Liberar':'Bloquear'} veículo · ${v.placa}</h3>
      ...
      <div style="background:rgba(46,107,255,.08);border:1px solid var(--line-2);border-radius:12px;padding:12px 14px;margin-bottom:14px">
  ...
  function reportarForm(locId){const l=byId(W().locatarios,locId);
    modal(`<h3>Reportar algo à VIZIO Frota</h3>
  ...
  function _enviarReport(locId){const dd=document.getElementById("rp_desc").value.trim();if(!dd){toast("Descreva o que aconteceu.");return;}
    ...
    modal(`<div style="text-align:center;padding:16px 6px">
      ...
      <p style="color:var(--sec);font-size:.92rem;max-width:360px;margin:0 auto 4px">Recebemos, ${l.nome.split(" ")[0]}. A equipe da VIZIO Frota foi notificada e vai te retornar em breve pelo WhatsApp. Você acompanha o andamento aqui no seu portal.</p>
  ...
  /* ---- Documentos premium (identidade VIZIO Frota) ---- */
  function printDoc(titulo,corpo,opt){opt=opt||{};const w=window.open("","_blank");
    w.document.write(`<!DOCTYPE html><html lang="pt-BR"><head><meta charset="utf-8"><title>${titulo} · VIZIO Frota</title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&display=swap" rel="stylesheet">
    <style>@page{margin:0}
      *{box-sizing:border-box}
      body{font-family:'Inter',Arial,sans-serif;color:#151A26;margin:0;font-size:12.5px}
      .page{padding:22mm 18mm 24mm;position:relative;min-height:100vh}
      .hd{display:flex;justify-content:space-between;align-items:center;border-bottom:1.5px solid #151A26;padding-bottom:14px}
      .hd img{height:42px}
      .hd .rt{font-size:10px;color:#8F8F96;text-align:right;letter-spacing:.02em;line-height:1.5}
      .acc{height:3px;background:linear-gradient(90deg,#2E6BFF,#46E0D0 55%,transparent);margin:0 0 22px;border-radius:2px}
      .doc-t{font-size:9px;letter-spacing:.32em;color:#2E6BFF;text-transform:uppercase;margin-top:20px}
      h1{font-size:22px;font-weight:600;margin:2px 0 6px;color:#0A0F1E;letter-spacing:-.01em}
      ...
      table.parts td.k{background:#F6F3EC;font-weight:600;color:#0A0F1E;width:26%;white-space:nowrap}
      .cl{margin:0 0 11px;line-height:1.7}
      .cl b{color:#0A0F1E}
      .num{color:#2E6BFF;font-weight:700}
      table.grid{width:100%;border-collapse:collapse;font-size:12px;margin:8px 0}
      table.grid th{background:#0A0F1E;color:#F8F8F8;text-align:left;padding:8px 10px;font-size:10px;letter-spacing:.05em;text-transform:uppercase;font-weight:600}
      ...
      .sign div{flex:1;text-align:center;border-top:1px solid #151A26;padding-top:7px;font-size:11px;color:#151A26}
      ...
      .wm{position:fixed;bottom:34%;left:50%;transform:translateX(-50%) rotate(-20deg);font-size:120px;font-weight:700;color:#2E6BFF;opacity:.05;letter-spacing:.1em;pointer-events:none}
    </style></head><body><div class="page">
    <div class="hd"><img src="viziofrota-preta.png" alt="VIZIO Frota" onerror="this.style.display='none'"/>
      <div class="rt">A INTELIGÊNCIA POR TRÁS DA FROTA<br>WhatsApp (77) 98113-2845 · viziofrota</div></div>
    <div class="acc"></div>
    ${opt.wm?'<div class="wm">VIZIO Frota</div>':''}
    <div class="doc-t">${opt.tag||"Documento"}</div>${corpo}
    <div class="ft"><span>VIZIO Frota · A inteligência por trás da frota</span><span>Sistema by VIZIO — um produto INPERSON · ${fmtDate("2026-07-12")}</span></div>
  ...
  function contrato(id){const k=W().contratos.find(x=>x.id===id);if(!k)return;const l=byId(W().locatarios,k.locatarioId),v=byId(W().veiculos,k.veiculoId);
    printDoc("Contrato de Locação",`
      ...
      <table class="parts">
        <tr><td class="k">Locadora</td><td>VIZIO Frota — A inteligência por trás da frota · Locação de Veículos · WhatsApp (77) 98113-2845</td></tr>
      ...
      <div class="sign"><div>${k.assinatura?`<img src="${k.assinatura.img}" style="max-height:46px;display:block;margin:-42px auto 4px"/><b>${l.nome}</b><br><span style="color:#8F8F96">assinado eletronicamente em ${fmtDate(k.assinatura.data)}</span>`:(l.nome||'Locatário(a)')+'<br><span style="color:#8F8F96">Locatário(a)</span>'}</div><div>VIZIO Frota<br><span style="color:#8F8F96">Locadora</span></div></div>`,
  ...
  function _sigInit(){const c=document.getElementById("sigpad");if(!c)return;const ctx=c.getContext("2d");ctx.lineWidth=2.6;ctx.lineCap="round";ctx.strokeStyle="#0A0F1E";let draw=false,px,py;
  ...
  function verMulta(id){const m=(W().multas||[]).find(x=>x.id===id);if(!m)return;const l=byId(W().locatarios,m.locatarioId),v=byId(W().veiculos,m.veiculoId);
    ...
    const acts=(l&&l.telefone?waBtn(l.telefone,`Olá ${(l.nome||"").split(" ")[0]}, VIZIO Frota. Sobre a multa ${m.infracao} (${fmtBRL(m.valor)})...`,"WhatsApp"):"")+`<button class="b-ghost" onclick="CRUD.open('multas','${m.id}')">Editar</button>`;
