/* =========================================================================
   MODO AURA — VIZIO Frota · "CONSTELAÇÃO" (v3, sem feixes/linhas)
   ...
   Acento configurável: window.VZ_ACCENT (default steel VIZIO Frota).
   ========================================================================= */
(function () {
  'use strict';
  ...
  function accentHex() {
    var v = window.VZ_ACCENT ||
      (getComputedStyle(document.documentElement).getPropertyValue('--steel') || '').trim() || '#2E6BFF';
    return v.charAt(0) === '#' ? v : '#2E6BFF';
  }
  function rgb(h){h=h.replace('#','');if(h.length===3)h=h.split('').map(function(c){return c+c;}).join('');
    var n=parseInt(h,16);return[(n>>16)&255,(n>>8)&255,n&255];}
  var HEX=accentHex(), R=rgb(HEX).join(','), RL=rgb('#46E0D0').join(',');
  ...
  css.textContent = [
    ...
    '[data-theme="light"] .vz-star{background:#2E6BFF}',
    ...
    /* Fix do drawer mobile (§2 do padrão): sem isto o AURA injeta z-index:45 e o
       ...
       abaixo de modal 50). VIZIO Frota usa left:-262/0 (não transform), então só
